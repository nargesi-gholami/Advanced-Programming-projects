...
Default_filter::Default_filter(hotel_vec all_hotels_, float average_, float standard_deviation_)
	: Filter(all_hotels_)
{
	average = average_;
...
hotel_vec Default_filter::apply()
{
	hotel_vec final_hotel;
	for (int i = 0 ; i < all_hotels.size() ; i++)
		if ((all_hotels[i]->get_price() - average) <= 2*standard_deviation &&
         (all_hotels[i]->get_price() - average) >= -2*standard_deviation )
    	final_hotel.push_back(all_hotels[i]);
	return final_hotel;
}
