...
void calculate_gpa(float& GPA, lesson_v student_grade, int semester_num);
...
void generate_output(vector <int> final_course, int semester_num);
void swap_vec(vector <int>& token_course, int j);
bool end_of_course(courseTable student_course);
void mark_read_course(courseTable& student_course, lesson_v student_grade, vector <int> final_course , int semester_num);
...
int main(int argc, char* argv[])
{
	...
	while (!end_of_course(student_course))
	{
		token_course.clear();
		final_course.clear();
		...
		calculate_gpa(GPA, student_grade, semester_num);
		...
		mark_read_course(student_course,student_grade , final_course , semester_num);


		sort_takeble_course(final_course, student_course, FOR_OUTPUT);
		generate_output(final_course, semester_num);
		semester_num ++;
	}
}
...
void mark_read_course(courseTable& student_course , lesson_v student_grade , vector <int> final_course , int semester_num)
{
	for (int i = 0; i < final_course.size(); i++)
		student_course[final_course[i] - 1].read = READ;
}
...
void read_schedule_two(ifstream& inputFile, course& current)
{
	...
	if (start != current.time.start)
	{
		...
		cout << current.time.start_exception;
	}
	getline(inputFile, temp, '-');
	getline(inputFile, temp, ':');
	int end = stoi(temp);
...
void read_schedule_three(ifstream& inputFile, course& current)
{
	...
	if (current.time.day_exception.compare(current.time.day3) && current.time.day_exception.compare(current.time.day2))
	{
		current.time.day_exception = EMPTY;
		current.time.start_exception = DONT_HAVE;
		current.time.end_exception = DONT_HAVE;
...
bool check_time(courseTable student_course, int course_number, int i , string time2)
{
...
bool checkClassTime(courseTable student_course, int course_number, int finall, string time1, string time2)
{
	if (!time1.compare(time2))
		if (time1 != NO_CLASS)
			if (check_time(student_course, course_number, finall,time2))
					return true;
